...
| 安全项 | 状态 |
|--------|------|
| **路径遍历防护** | ✅ 音频/OCR/转写端点绝对路径验证 |
| **上传文件名清洗** | ✅ 剥离目录组件、拒绝 `..` |
| CORS 配置加固 | ✅ 生产环境强制验证 |
| 安全响应头 | ✅ CSP, HSTS, X-Frame-Options (Nginx) |
| JWT 密钥验证 | ✅ 生产环境强制要求 |
| SQL 注入防护 | ✅ 参数化查询 |
| XSS 防护 | ✅ 输入验证 + 转义 |
| **RBAC 强制执行** | ✅ 管理端点权限不可用时返回 503 |
| **连接复用** | ✅ LLM API 客户端 aiohttp 会话复用 |
| **错误信息脱敏** | ✅ API 错误不暴露内部详情 |

详见：[安全审计报告 2026-04-05](docs/SECURITY_AUDIT_2026-04-05.md)
...
- 🔒 **CRITICAL**: 修复路径遍历漏洞 — audio/annotation 端点绝对路径验证
- 🔒 **CRITICAL**: 修复上传文件名路径遍历 — 剥离目录组件
- 🔒 **CRITICAL**: 修复 aiohttp 连接泄露 — 会话复用
- 🛡️ **HIGH**: 修复 RBAC 静默降级 — 管理端点权限不可用时返回 503
- 🔧 **MEDIUM**: 修复闭包变量捕获 bug — 向量化批次处理
- 🔧 **MEDIUM**: Nginx 安全响应头（CSP, X-Frame-Options 等）
- 🔧 **MEDIUM**: 移除硬编码路径 — discuss.py 使用环境变量
- ✅ 877 测试通过

...
const GuoxueModule = {
    ...
    async loadStats() {
        try {
            ...
            if (statsDiv) {
                statsDiv.innerHTML = `
                    <span class="stat-badge">📚 ${Utils.escapeHtml(String(data.data.book_count || 0))} 部典籍</span>
                    <span class="stat-badge">📄 ${Utils.escapeHtml(String(Utils.formatNumber(data.data.content_count || 0)))} 条内容</span>
                `;
            }
        } catch (error) {
    ...
    renderBookList(books) {
        ...
        listDiv.innerHTML = books.map(book => `
            <div class="guoxue-book-item" data-book-id="${Utils.escapeHtml(String(book.book_id))}">
                <span class="book-title">${Utils.escapeHtml(book.title)}</span>
                <span class="book-count">${Utils.escapeHtml(String(book.content_count))}</span>
            </div>
        `).join('');
    ...
    async loadChapters(bookId, page = 1) {
        ...
        try {
            const data = await API.guoxue.getChapters(bookId, { size: 100, page });
            const chapters = data.data.results;
            const totalPages = Math.ceil(data.data.total / 100);
            ...
            const chapterList = contentDiv.querySelector('.chapter-list');
            const itemHtml = chapters.map(ch => `
                <div class="chapter-item" data-id="${Utils.escapeHtml(String(ch.id))}">
                    <span class="chapter-num">第${Utils.escapeHtml(String(ch.chapter_id))}章</span>
                    <span class="chapter-length">${Utils.escapeHtml(String(ch.body_length))}字</span>
                </div>
            `).join('');
            ...
            const paginationDiv = contentDiv.querySelector('.chapter-pagination');
            if (hasMore) {
                paginationDiv.innerHTML = `
                    <div style="text-align:center;padding:12px;">
                        <button class="btn btn-secondary guoxue-load-more-btn">加载更多（${page}/${totalPages}页）</button>
                    </div>
                `;
                paginationDiv.querySelector('.guoxue-load-more-btn').addEventListener('click', function() {
                    this.disabled = true;
                    this.textContent = '加载中...';
                    GuoxueModule.loadChapters(bookId, page + 1);
                });
            } else if (totalPages > 1) {
                paginationDiv.innerHTML = `<div style="text-align:center;padding:8px;color:#666;">已加载全部 ${data.data.total} 条</div>`;
            } else {
                paginationDiv.innerHTML = '';
            }

            chapterList.querySelectorAll('.chapter-item:not([data-bound])').forEach(el => {
            ...
            contentDiv.innerHTML = `<p class="text-small">加载失败：${Utils.escapeHtml(error.message)}</p>`;
        }
    },
    ...
    showContentModal(contentData) {
        ...
        modalBody.innerHTML = `
            <button class="modal-close" id="guoxueModalClose">×</button>
            <h2>第${contentData.chapter_id}章</h2>
            <p class="text-small text-muted">${contentData.body_length}字</p>
            <div class="guoxue-body">${Utils.escapeHtml(contentData.body).replace(/\n/g, '<br>')}</div>
        `;

        modal.querySelector('#guoxueModalClose').addEventListener('click', closeAllModals);
        ...
        try {
            ...
            contentDiv.innerHTML = `
                ...
                <div class="search-results">
                    ${results.map(r => `
                        <div class="search-result-item" data-id="${Utils.escapeHtml(String(r.id))}">
                            <div class="result-meta">
                                <span>${Utils.escapeHtml(r.book_title)}</span>
                                <span>第${Utils.escapeHtml(String(r.chapter_id))}章</span>
                            </div>
            ...
            contentDiv.innerHTML = `<p class="text-small">搜索失败：${Utils.escapeHtml(error.message)}</p>`;
        }
    }
};

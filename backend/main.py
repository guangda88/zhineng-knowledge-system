...
# 导入推理服务
from services.reasoning import CoTReasoner, ReActReasoner, GraphRAGReasoner

# 导入领域系统 (P5)
from domains import setup_domains, get_registry
from domains.base import DomainType

# 导入API网关 (P5)
from gateway import APIGateway, InMemoryRateLimiter, CircuitBreakerRegistry
...
async def create_document(doc: DocumentCreate) -> Dict[str, Any]:
    ...
    doc_id = await pool.fetchval(
        ...
        doc.title, doc.content, doc.category, json.dumps(doc.tags)
    )
...
@app.post("/api/v1/embeddings/update", response_model=Dict[str, Any])
...
@app.get("/api/v1/retrieval/status", response_model=Dict[str, Any])
...
async def get_cot_reasoner() -> CoTReasoner:
    ...
    if _cot_reasoner is None:
        ...
        if not api_key:
            raise ValueError("DEEPSEEK_API_KEY environment variable is required for reasoning service")
...
@app.post("/api/v1/reason", response_model=Dict[str, Any])
...
@app.post("/api/v1/graph/query", response_model=Dict[str, Any])
...
@app.get("/api/v1/graph/data", response_model=Dict[str, Any])
...
@app.post("/api/v1/graph/build", response_model=Dict[str, Any])
async def build_graph(category: Optional[str] = None) -> Dict[str, Any]:
    ...
    if category:
        rows = await pool.fetch(
            "SELECT id, title, content FROM documents WHERE category = $1 LIMIT 50",
            category
        )
    else:
        rows = await pool.fetch(
            "SELECT id, title, content FROM documents LIMIT 100"
        )
...
@app.get("/api/v1/reasoning/status", response_model=Dict[str, Any])
...
@app.post("/api/v1/gateway/query", response_model=Dict[str, Any])
...
@app.get("/api/v1/domains/{domain_name}/stats", response_model=Dict[str, Any])
...
@app.post("/api/v1/domains/{domain_name}/query", response_model=Dict[str, Any])
...
@app.get("/api/v1/health/{check_name}", response_model=Dict[str, Any])
...
@app.get("/api/v1/gateway/stats", response_model=Dict[str, Any])

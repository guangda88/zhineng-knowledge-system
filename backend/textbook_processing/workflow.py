...
# 9本大专教材定义
TEXTBOOKS = [
    TextbookInfo(number=1, title="智能气功科学概论", short_name="概论"),
    TextbookInfo(number=2, title="智能气功科学精义", short_name="精义"),
    TextbookInfo(number=3, title="智能气功科学混元整体理论", short_name="混元整体理论"),
    TextbookInfo(number=4, title="智能气功科学功法学", short_name="功法学"),
    TextbookInfo(number=5, title="智能气功科学超常智能", short_name="超常智能"),
    TextbookInfo(number=6, title="智能气功科学传统气功知识综述", short_name="传统气功知识综述"),
    TextbookInfo(number=7, title="智能气功科学气功与人类文化", short_name="气功与人类文化"),
    TextbookInfo(number=8, title="中国气功发展简史", short_name="中国气功发展简史"),
    TextbookInfo(number=9, title="智能气功科学的现代科学研究", short_name="气功的现代科学研究"),
]
...
class LingFlowWorkflow:
        ...
        try:
            ...
            if catalog_path.exists():
                ...
                for textbook in self.textbooks:
                    # 设置处理后目录（无论是否找到文件）
                    dir_name = (
                        f"0{textbook.number}-{textbook.short_name}"
                        if textbook.number < 10
                        else f"{textbook.number}-{textbook.short_name}"
                    )
                    ...
                    if pdf_files:
                        ...
                        textbook.txt_path = (
                            self.project_root / pdf_files.get("txt")
                            if pdf_files.get("txt")
                            else None
                        )
                        textbook.docx_path = (
                            self.project_root / pdf_files.get("docx")
                            if pdf_files.get("docx")
                            else None
                        )
                        textbook.source_dir = (
                            textbook.pdf_path.parent if textbook.pdf_path else None
                        )

                result.data = {
                    "textbooks_found": sum(
                        1 for t in self.textbooks if t.pdf_path and t.pdf_path.exists()
                    ),
        ...
        try:
            ...
            for textbook in self.textbooks:
                ...
                try:
                    ...
                    parsed_count += 1
                    logger.info(
                        f"    解析到 {len(parse_result.items)} 个条目，置信度: {parse_result.confidence}"
                    )
    ...
    def run(self, steps: Optional[List[int]] = None, force_extract: bool = False) -> Dict:
        """运行工作流
    ...
    def generate_report(self) -> Dict:
        ...
        report = {
            ...
            "summary": {
                ...
                "failed": failed,
                "success_rate": (
                    f"{completed / len(self.steps_results) * 100:.1f}%"
                    if self.steps_results
                    else "N/A"
                ),
            },

...
class RedisConfig(BaseSettings):
    ...
    # 连接池配置
    REDIS_POOL_SIZE: int = Field(default=10, ge=1, le=50, description="Redis连接池大小")
    REDIS_SOCKET_TIMEOUT: int = Field(default=5, ge=1, le=30, description="Redis连接超时（秒）")
    REDIS_SOCKET_CONNECT_TIMEOUT: int = Field(
        default=5, ge=1, le=30, description="Redis连接超时（秒）"
    )

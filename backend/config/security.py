...
class SecurityConfig(BaseSettings):
    ...
    ALLOWED_CREDENTIALS: bool = Field(default=True, description="允许携带凭证")
    ...
    BGE_API_KEY: Optional[str] = Field(default=None, description="BGE API密钥")
    ...
    JWT_EXPIRATION: int = Field(default=3600, ge=60, description="JWT过期时间（秒）")
    ...
    RATE_LIMIT_PER_HOUR: int = Field(default=1000, ge=1, description="每小时请求限制")

    # 安全头
    ENABLE_SECURITY_HEADERS: bool = Field(default=True, description="是否启用安全头")

    # 管理API密钥（逗号分隔字符串）
    ADMIN_API_KEYS: str = Field(default="", description="管理端点API密钥，逗号分隔")

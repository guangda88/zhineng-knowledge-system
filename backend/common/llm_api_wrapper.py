    ...
    async def call_api(
        ...
        **kwargs
        ...
        for attempt in range(self.RETRY_CONFIG["max_retries"]):
            try:
                return await self._attempt_call(messages, temperature, max_tokens, timeout, **kwargs)
                ...
                if attempt < self.RETRY_CONFIG["max_retries"] - 1:
                    ...
                    logger.warning(f"Rate limit hit (attempt {attempt + 1}), retrying in {delay:.1f}s")
                    await asyncio.sleep(delay)
                else:
                    logger.error(f"Rate limit retry exhausted after {self.RETRY_CONFIG['max_retries']} attempts")
...
def get_llm_client(
    ...
    if _global_client is None:
        ...
        _global_client = LLMAPIClient(
            api_key=api_key,
            api_url=api_url or "https://open.bigmodel.cn/api/paas/v4/chat/completions",
...
if __name__ == "__main__":
    ...
    async def example_usage():
        ...
        messages = [
            {"role": "user", "content": "你好，请介绍一下灵知系统"}
        ]

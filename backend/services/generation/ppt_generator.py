...
from .base import BaseGenerator, GenerationRequest, GenerationResult, GenerationStatus
...
class PPTGenerator(BaseGenerator):
    ...
    async def generate(self, request: GenerationRequest) -> GenerationResult:
        ...
        try:
            ...
            ppt_structure = await self._build_ppt_structure(
                topic=request.topic, slide_count=slide_count, style=style, language=language
            )

            # 生成PPT文件
            output_path = await self._create_ppt(
                task_id=request.task_id, structure=ppt_structure, theme=theme
            )
    ...
    async def _build_ppt_structure(
        self, topic: str, slide_count: int, style: str, language: str
    ) -> Dict[str, Any]:
        """构建PPT结构"""
        ...
        if pool:
            try:
                ...
                logger.warning(f"PPT generation search failed: {e}")
    ...
    def _extract_bullet_points(self, content: str, max_points: int = 5) -> List[str]:
        ...
        if not bullet_points:
            bullet_points = ["核心概念解析", "理论与实践结合", "重点要点说明", "注意事项提醒"]

        return bullet_points

    async def _create_ppt(self, task_id: str, structure: Dict[str, Any], theme: str) -> str:
        """创建PPT文件"""
    ...
    async def generate_course_ppt(
        self, course_title: str, chapters: List[str], style: str = "teaching"
    ) -> str:
        """生成课程PPT"""

        structure = {"title": course_title, "type": "course", "slides": []}
        ...
        for chapter in chapters:
            ...
            if pool:
                try:
                    ...
                    logger.warning(f"Chapter content search failed: {e}")

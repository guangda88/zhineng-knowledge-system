...
class TextbookImporter:
    ...
    async def _import_node_recursive(
        ...
        if parent_id:
            async def get_parent_path():
                row = await conn.fetchrow(
                    "SELECT path FROM textbook_nodes WHERE id = $1",
                    parent_id
                )
        ...
        # 插入节点
        metadata = {
            'source': 'toc_ai_generated',
            'line_range': line_range  # 存储在metadata中
        }
...
async def main():
    ...
    try:
        ...
        # 导入TOC
        stats = await importer.import_toc(textbook_id, toc_path, text_path)

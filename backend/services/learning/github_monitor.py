...
class GitHubMonitorService:
            ...
            for repo_info in self.MONITORED_REPOS:
                try:
                    repo_updates = await self._fetch_repo_updates(
                        session, repo_info["owner"], repo_info["repo"], since_date
                    )

                    for update in repo_updates:
                        update.relevance = self._calculate_relevance(update, repo_info["relevance"])
    ...
    async def _fetch_repo_updates(
        self, session: "aiohttp.ClientSession", owner: str, repo: str, since_date: datetime
    ) -> List[TechUpdate]:
        """获取仓库更新"""
        updates = []
        ...
        params = {"since": since_date.isoformat(), "per_page": 20}
    ...
    async def suggest_innovations(self) -> List[Dict[str, Any]]:
        ...
        # 过滤高相关性更新
        high_relevance_updates = [u for u in updates if u.relevance > 0.7]

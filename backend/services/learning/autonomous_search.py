...
class AutonomousSearchService:
    ...
    async def search_until_satisfied(
        self, question: str, max_rounds: int = 3, confidence_threshold: float = 0.7
    ) -> Dict[str, Any]:
        """搜索直到获得满意答案"""
    ...
    def _evaluate_results(self, results: List[SearchResult], question: str) -> Dict[str, Any]:
        """评估搜索结果"""
        if not results:
            return {"result": None, "confidence": 0.0, "reason": "没有找到相关结果"}
        ...
        for result in results:
            ...
            # 综合评分
            score = overlap * 0.5 + result.relevance_score * 0.3 + result.confidence * 0.2

            if score > best_score:
                best_score = score
                best_result = result

        return {"result": best_result, "confidence": best_score, "reason": "相关性评分"}

    def _synthesize_answer(
        self, question: str, all_results: List[SearchResult], best_result: Dict[str, Any]
    ) -> str:
        """综合生成答案"""

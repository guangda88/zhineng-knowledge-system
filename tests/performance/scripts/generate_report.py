...
def parse_stats_file(csv_file: Path) -> Dict[str, Any]:
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            stats['total']['avg_response_time'] += float(row['Average Response Time']) * int(row['Request Count'])
            stats['total']['min_response_time'] = min(stats['total']['min_response_time'], float(row['Min Response Time']))
            stats['total']['max_response_time'] = max(stats['total']['max_response_time'], float(row['Max Response Time']))
...
def main():
    ...
    parser.add_argument(
        'prefix',
        help='Locust CSV 文件前缀 (如: reports/standard_20240101_120000)'
    )
    parser.add_argument(
        '-o', '--output',
        help='输出报告文件 (默认: PREFIX_report.md)',
        default=None
    )

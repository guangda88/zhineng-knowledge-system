...
app = FastAPI(
    title="气功知识服务", description="Zhinqong (Qigong) Knowledge Domain Service", version="1.0.0"
)
...
async def get_categories():
    ...
    for doc in QIGONG_KNOWLEDGE_BASE:
        ...
        if cat not in categories:
            categories[cat] = []
        categories[cat].append({"id": doc["id"], "title": doc["title"], "tags": doc["tags"]})
    return categories
...
async def search_knowledge(request: QueryRequest):
    ...
    # 按相关性排序（简单实现）
    results.sort(
        key=lambda x: (query in x.title.lower(), sum(query in tag.lower() for tag in x.tags)),
        reverse=True,
    )

    return QueryResponse(
        query=request.query, documents=results[: request.top_k], total=len(results), domain="qigong"
    )
...
async def question_answering(question: str, reasoning: bool = False):
    ...
    for doc in QIGONG_KNOWLEDGE_BASE:
        if query_lower in doc["title"].lower() or any(
            ...
            for keyword in ["气功", "高血压", "练习", "呼吸", "八段锦", "太极拳"]
